...
transformation_list = [cv2.MORPH_CLOSE, cv2.MORPH_OPEN]
figure_name_list = ['output/captcha/' + ele + '.png'
                    for ele in ['close', 'open']]
...
bin_img_list = [cv2.threshold(img, 127, 255, cv2.THRESH_BINARY_INV)[1]
                for img in img_list]

# plot_img_list(bin_img_list,
#               figure_name='dataset/captcha/original.png',
#               show_me=False)
...
for transformation, figure_name in zip(transformation_list, figure_name_list):
    print('===')
    print(figure_name)
    app_img_list = [cv2.morphologyEx(img, transformation, kernel)
                    for img in bin_img_list]
    plot_img_list(app_img_list, figure_name=None, show_me=True)
