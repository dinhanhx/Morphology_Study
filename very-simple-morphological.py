...
baw_img = cv2.imread(r'46913809661_f7f7c35fe3_o.jpg', cv2.IMREAD_GRAYSCALE)
# any image
...
def morph (baw_image, strel, type = None):
    ...
    baw_image = np.insert(baw_image, np.zeros(nbh_row), np.nan, axis = 0)
    ...
    for i in range(col):
        for j in range(row):
            ...
            if type == "dil": new_image[i, j] = np.nanmin(sample)
            if type == "ero": new_image[i, j] = np.nanmax(sample)
    return new_image.astype(int)

new_img = morph(baw_img, strel, type="ero")
